...
void initEEPROM()
{
  ...
  writeEEPROMByte(eeUseExRoster, USE_EX_ROSTER);
...
void saveSelected()
{
  ...
  for(uint8_t r = 0; r < tCount; r++)
  {
    for (uint16_t s = selAddress; s < (selAddress+numLocoSlots); s++)
    {
      ...
      Console.printf("Saving Throttle: %d Address: %d ID: %d\r\n", r, s, selectedIDs[r][s-selAddress]);
    }
...
void restoreSelected()
{
  ...
  for(uint8_t r = 0; r < tCount; r++)
  {
    for(uint16_t s = selAddress; s < (selAddress + numLocoSlots); s++)
    {
      ...
      Console.printf("Restoring Throttle: %d Address: %d ID: %d\r\n", r, s, readEEPROMByte(s));
    }
    selAddress = selAddress + numLocoSlots;
  }
}
