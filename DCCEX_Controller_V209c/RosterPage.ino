...
void rosterPage(uint8_t button)
{
  ...
  switch(button)
  {
    ...
    case SelectButton:
    {
      ...
      {
        if(rosterList[selectedLocoID] == 0) return;
      }
      selectedIDs[thNum][activeSlot[thNum]] = selectedLocoID;   //selectedLocoID = Actual LocoID Being worked on
      if(readEEPROMByte(eeUseExRoster) == 0)
      { 
        throttles[activeSlot[thNum]]->setLoco(new Loco(readLocoAddress(selectedLocoID), LocoSource::LocoSourceEntry));
      }else
      {
        throttles[activeSlot[thNum]]->setLoco(new Loco(rosterList[selectedLocoID], LocoSource::LocoSourceRoster));
      }
...
void rosterDrawPage(uint8_t startID)
{
  ...
  {
    nextionCommand("l" + String(r) + ".bco=50712");       //Set all text Backgrounds to default Grey 
    ...
    if(readEEPROMByte(eeUseExRoster) == 0)   //If Don't Use DCCEX Roster
    {
    ...
    }else   //Use DCCEX Roster
    { 
      if((startID+r)<lastEntry)
//      if(rosterList[startID + r] !=0)
      { 
        Loco *loco = dccexProtocol.findLocoInRoster(rosterList[startID+r]);
        char *name = loco->getName();
        int address = loco->getAddress();
        nextionSetText("l" + String(r), name);
        nextionSetText("n" + String(r),"");
        nextionSetText("t" + String(r),""); 
        nextionSetText("r" + String(r),"");
        nextionSetText("a" + String(r), String(address));
      }else
      {
        nextionSetText("l" + String(r),"");
        nextionSetText("n" + String(r),"");
