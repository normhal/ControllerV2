...
//      auto *activeLoco = throttles[activeSlot];
      throttles[activeSlot]->setLoco(new Loco(guestAddr, LocoSource::LocoSourceEntry));
//      dccexProtocol.requestLocoUpdate(activeLoco->getAddress());

      auto th = throttles[activeSlot];
      Loco *activeLoco = th->getLoco();
      dccexProtocol.requestLocoUpdate(activeLoco->getAddress());
//      selectedIDs[thNum][activeSlot] = 
...
//************************************************************************
      if(selectedIDs[thNum][activeSlot] != 255)        
      {
        if(button >= FunctionSlotStart && button < (FunctionSlotStart + functionsPerPage))
        {
          ...
          if(readEEPROMByte(eeUseExRoster) == 0)
          {
            funcNum = readEEPROMByte(locoFuncBase + (selectedIDs[thNum][activeSlot]*fBlockSize) +(g_fSlot*2));    //retrieve the actual function number from its EEPROM slot
            actualFunc = funcNum & 0x7F;
            if(actualFunc == 127) break;       //Inactive/unassigned function
            funcImg = readEEPROMByte(locoFuncBase + (selectedIDs[thNum][activeSlot]*fBlockSize) +(g_fSlot*2)+1);
          }else
          {
            actualFunc = g_fSlot;
      ...
      if(i != activeSlot)
      {
        if(selectedIDs[thNum][i] != 255)
        {
          ...
          }else                                       //locoAddress != 0
          {
            ...
            nextionCommand("v" + String(i) + ".pco=" + String(USER1));
            if(readEEPROMByte(eeUseExRoster) == 0)
            {
            ...
            {
              nextionSetText("t" + String(i), String(readLocoAddress(selectedIDs[thNum][i])));
           }
          }
        }else //selectedID == 255 
        {
